...
setup_ds () {
	if ! pkg_install 389-ds; then
		echo 'Failed to find 389-ds package.'
		exit 1
	fi
...
setup_pki () {
	...
	# Ubuntu forgets to install this
	pkg_install libstax-java
...
setup_firewall () {
	if which ufw >/dev/null 2>/dev/null; then
...
setup_cockpit () {
	if dpkg -s cockpit >/dev/null 2>/dev/null; then
