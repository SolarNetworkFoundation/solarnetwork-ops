...
/**
 * Migrate a single `da_loc_datum` row into the `solardatm.da_datm` table.
 *
 * This function accepts the "current" property name arrays as an optimisation when migrating
 * multiple rows of data. If the given datum has properties that are not found in any of the
 * given name arrays, the name will be added and then the associated `solardatm.da_datm_meta`
...
BEGIN
	...
	FOR p IN SELECT * FROM jsonb_each_text(d.jdata_i) LOOP
		...
		BEGIN
			v_i[idx] := p.value::numeric;
		EXCEPTION WHEN others THEN
			RAISE WARNING 'JSON value not numeric: loc %, source %, ts %, i.key %, i.value %',
				d.loc_id, d.source_id, d.ts, p.key, p.value;
			v_i[idx] := NULL;
		END;
	END LOOP;
...
BEGIN
	...
	LOOP
		FETCH curs INTO d;
		EXIT WHEN NOT FOUND;
		SELECT * FROM solardatm.migrate_loc_datum(sid, d, p_i, p_a, p_s)
		INTO p_i, p_a, p_s;
		rcount := rcount + 1;
	END LOOP;
	CLOSE curs;
