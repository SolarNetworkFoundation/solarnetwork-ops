...
SELECT stmt || ';' AS stmt
FROM (SELECT unnest(ARRAY['_timescaledb_solarnetwork', 'solarbill', 'solardatm', 'solardnp3', 'solarev', 'solarnet', 'solaroscp', 'solaruser']) AS schem) AS s,
...
BEGIN
	FOR stmt IN
		SELECT res.stmt
		FROM (SELECT unnest(ARRAY['solarbill', 'solarcommon', 'solardatm', 'solardnp3', 'solarev', 'solarnet', 'solaroscp', 'solaruser']) AS schem) AS s,
