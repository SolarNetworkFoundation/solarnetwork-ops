...
SELECT stmt || ';' AS stmt
FROM (SELECT unnest(ARRAY['_timescaledb_solarnetwork', 'quartz', 'solaragg', 'solarbill', 'solardatum', 'solarev', 'solarnet', 'solaruser']) AS schem) AS s,
...
BEGIN
	FOR stmt IN
		SELECT res.stmt
		FROM (SELECT unnest(ARRAY['solaragg', 'solarbill', 'solarcommon', 'solardatum', 'solarev', 'solarnet', 'solaruser']) AS schem) AS s,
