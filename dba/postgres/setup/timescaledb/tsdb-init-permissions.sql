...
SELECT stmt || ';' AS stmt
FROM (SELECT unnest(ARRAY['_timescaledb_solarnetwork', 'quartz', 'solarbill', 'solardatm', 'solarev', 'solarnet', 'solaroscp', 'solaruser']) AS schem) AS s,
...
BEGIN
	FOR stmt IN
		SELECT res.stmt
		FROM (SELECT unnest(ARRAY['solarbill', 'solarcommon', 'solardatm', 'solarev', 'solarnet', 'solaroscp', 'solaruser']) AS schem) AS s,
