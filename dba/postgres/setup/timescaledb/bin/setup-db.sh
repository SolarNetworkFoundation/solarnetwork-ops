...
if [ -n "$DRY_RUN" ]; then
	...
	psql $PSQL_CONN_ARGS -U $PG_ADMIN_USER -d $PG_DB -c "CREATE EXTENSION IF NOT EXISTS aggs_for_vecs WITH SCHEMA public" || exit 10
fi
...
if [ -n "$DRY_RUN" ]; then
	echo "psql $PSQL_CONN_ARGS -U $PG_ADMIN_USER -d $PG_DB -c 'SELECT res.* FROM (SELECT unnest(ARRAY['_timescaledb_solarnetwork', 'solarcommon', 'solarbill', 'solardatm', 'solardin', 'solardnp3', 'solarev', 'solarnet', 'solaroscp', 'solaruser']) AS schem) AS s, LATERAL (SELECT * FROM public.set_ownership(s.schem, '$PG_DB_OWNER')) AS res'"
else
	psql $PSQL_CONN_ARGS -U $PG_ADMIN_USER -d $PG_DB -P pager=off -qAtc "SELECT stmt || ';' FROM (SELECT unnest(ARRAY['_timescaledb_solarnetwork', 'solarcommon', 'solarbill', 'solardatm', 'solardin', 'solardnp3', 'solarev', 'solarnet', 'solaroscp', 'solaruser']) AS schem) AS s, LATERAL (SELECT * FROM public.set_ownership(s.schem, '$PG_DB_OWNER')) AS res" || exit 11
fi
...
if [ -n "$DRY_RUN" ]; then
	...
	psql $PSQL_CONN_ARGS -U $PG_ADMIN_USER -d $PG_DB -P pager=off -qAt <<-EOF
		...
		SELECT _timescaledb_solarnetwork.change_to_hypertable('solardatm',	'aud_user_io',			'ts_start',	'1 years'${INDEX_TABLESPACE:+,'$INDEX_TABLESPACE'});
		SELECT _timescaledb_solarnetwork.change_to_hypertable('solardatm',	'aud_user_daily',		'ts_start',	'5 years'${INDEX_TABLESPACE:+,'$INDEX_TABLESPACE'});
		SELECT _timescaledb_solarnetwork.change_to_hypertable('solardatm',	'aud_user_monthly',		'ts_start',	'10 years'${INDEX_TABLESPACE:+,'$INDEX_TABLESPACE'});
...
if [ -n "$INDEX_TABLESPACE" ]; then
	...
	if [ -n "$DRY_RUN" ]; then
		...
		psql $PSQL_CONN_ARGS -U $PG_ADMIN_USER -d $PG_DB -P pager=off -At <<-EOF
			SELECT stmt || ';' FROM (SELECT unnest(ARRAY['solarbill', 'solardatm', 'solardin', 'solardnp3', 'solarev', 'solarnet', 'solaroscp', 'solaruser']) AS schem) AS s,
